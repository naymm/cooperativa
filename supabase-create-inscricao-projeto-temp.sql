-- ========================================
-- CRIAÇÃO DA TABELA INSCRICAO_PROJETO (VERSÃO TEMPORÁRIA)
-- ========================================
-- ⚠️ IMPORTANTE: Execute primeiro o script verificar-colunas-projetos.sql
-- para identificar os nomes corretos das colunas da tabela projetos
...
-- ========================================
-- FUNÇÕES ÚTEIS (VERSÃO TEMPORÁRIA)
-- ========================================

-- ⚠️ ATENÇÃO: Ajuste os nomes das colunas conforme a estrutura da sua tabela projetos
-- Execute primeiro: SELECT * FROM projetos LIMIT 1; para ver as colunas
...
-- Função para buscar inscrições pendentes (VERSÃO TEMPORÁRIA)
-- ⚠️ AJUSTE: Substitua 'nome' pela coluna correta da sua tabela projetos
...
BEGIN
    ...
    SELECT 
        ...
        -- ⚠️ AJUSTE: Substitua 'nome' pela coluna correta da sua tabela projetos
        -- Possíveis opções: p.name, p.title, p.titulo, p.descricao, etc.
        p.nome as projeto_nome  -- ← AJUSTE ESTA LINHA
    FROM inscricao_projeto ip
...
-- View para inscrições com dados completos (VERSÃO TEMPORÁRIA)
-- ⚠️ AJUSTE: Substitua os nomes das colunas conforme sua tabela projetos
...
SELECT 
    ...
    -- ⚠️ AJUSTE: Substitua os nomes das colunas conforme sua tabela projetos
    p.nome as projeto_nome,           -- ← AJUSTE ESTA LINHA
    p.tipo as projeto_tipo,           -- ← AJUSTE ESTA LINHA
    p.provincia as projeto_provincia, -- ← AJUSTE ESTA LINHA
    p.municipio as projeto_municipio, -- ← AJUSTE ESTA LINHA
    p.preco_final as projeto_preco    -- ← AJUSTE ESTA LINHA
FROM inscricao_projeto ip
JOIN cooperados c ON c.id = ip.cooperado_id
JOIN projetos p ON p.id = ip.project_id;
...
BEGIN
    ...
    RAISE NOTICE '⚠️ ATENÇÃO: Ajuste os nomes das colunas nas funções e views!';
    RAISE NOTICE 'Execute: SELECT * FROM projetos LIMIT 1; para ver as colunas corretas.';
